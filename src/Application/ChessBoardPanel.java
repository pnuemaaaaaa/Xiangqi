...
public class ChessBoardPanel extends JPanel {
    ...
    public ChessBoardPanel(ChessBoardModel model) {
        ...
        setPreferredSize(new Dimension(
                CELL_SIZE * (ChessBoardModel.getX() - 1) + MARGIN * 2,
                CELL_SIZE * (ChessBoardModel.getY() - 1) + MARGIN * 2
        ));
    ...
    private void handleMouseClick(int x, int y) {
        int newX = Math.round((float)(x - MARGIN) / CELL_SIZE);
        int newY = Math.round((float)(y - MARGIN) / CELL_SIZE);

        if (!model.isValidPosition(newX,newY)) {
            return;
        }

        if (selectedPiece == null) {
            selectedPiece = model.getPieceAt(newX, newY);
        } else {
            model.movePiece(selectedPiece, newX, newY,model.getPieces());
            selectedPiece = null;
        }
    ...
    private void drawBoard(Graphics2D g) {
        ...
        // 绘制横线
        for (int i = 0; i < ChessBoardModel.getX(); i++) {
            int y = MARGIN + i * CELL_SIZE;
            g.drawLine(MARGIN, y, MARGIN + (ChessBoardModel.getX() - 1) * CELL_SIZE, y);
        }

        // 绘制竖线
        for (int i = 0; i < ChessBoardModel.getX(); i++) {
            int x = MARGIN + i * CELL_SIZE;
            if (i == 0 || i == ChessBoardModel.getX() - 1) {
                // 两边的竖线贯通整个棋盘
                g.drawLine(x, MARGIN, x, MARGIN + (ChessBoardModel.getX() - 1) * CELL_SIZE);
            } else {
                // 中间的竖线分为上下两段（楚河汉界断开）
                g.drawLine(x, MARGIN, x, MARGIN + 4 * CELL_SIZE);
                g.drawLine(x, MARGIN + 5 * CELL_SIZE, x, MARGIN + (ChessBoardModel.getY() - 1) * CELL_SIZE);
            }
        }
    ...
    private void drawPieces(Graphics2D g) {
        ...
        for (Piece piece : model.getPieces()) {
            ...
            int y = MARGIN + piece.getPosition().getY() * CELL_SIZE;
